...
interface LogonOption { value: LogonType; label: string }
...
export type ConnReq = {
  protocol: Protocol; host: string; port: number; username: string;
  logonType: LogonType; password?: string; privateKey?: string; passphrase?: string;
...
export function ConnectionBar({ onConnect, onSave, connecting, saving }: ConnectionBarProps) {
  ...
  const browseKey = async () => {
    ...
    if (!path) return;
    try { const c = await api.readKeyFile(path); setKeyPath(path); setPrivateKey(c); }
    catch { alert("Could not read key file — check the path."); }
  };
  ...
  const req = (): ConnReq => ({
    ...
    logonType,
    password: (logonType === "normal" || logonType === "interactive") && password ? password : undefined,
  ...
  return (
    <form
      ...
      onSubmit={(e) => { e.preventDefault(); onConnect(req()); }}
    >
      {/* Protocol */}
      ...
      {showEnc && (
        <Field label="Encryption" className="shrink-0">
          ...
          >
            {FTP_ENCRYPTION.map((e) => <option key={e.value} value={e.value}>{e.label}</option>)}
          </select>
        </Field>
      )}
      ...
      <Field label="Host / IP" className="flex-1 min-w-[7rem]">
        <Input className="h-8 w-full text-xs" value={host} onChange={(e) => setHost(e.target.value)} placeholder="hostname or IP" required />
      </Field>
      ...
      <Field label="Logon" className="shrink-0">
        ...
        >
          {logonOpts.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
        </select>
      </Field>
      ...
      {showUser && (
        <Field label="Username" className="shrink-0">
          <Input className="h-8 w-44 text-xs" value={username} onChange={(e) => setUsername(e.target.value)} placeholder="username" autoComplete="username" />
        </Field>
      )}
      ...
      {showPass && (
        <Field label="Password" className="shrink-0">
          <Input type="password" className="h-8 w-44 text-xs" value={password} onChange={(e) => setPassword(e.target.value)} autoComplete="current-password" />
        </Field>
      )}
        ...
        <>
          <Field label="Key file" className="shrink-0 w-36">
            <div className="flex gap-1">
              <Input className="h-8 flex-1 min-w-0 text-xs font-mono" value={keyPath} onChange={(e) => setKeyPath(e.target.value)} placeholder="~/.ssh/id_rsa" />
              <button type="button" onClick={browseKey} className="flex h-8 w-7 shrink-0 items-center justify-center rounded border border-border bg-surface text-subtle hover:bg-muted">
                <FolderOpen size={12} />
              </button>
            </div>
          </Field>
          <Field label="Passphrase" className="shrink-0">
            <Input type="password" className="h-8 w-20 text-xs" value={passphrase} onChange={(e) => setPassphrase(e.target.value)} placeholder="optional" />
          </Field>
        </>
      )}
      ...
      {/* Actions */}
      {onSave && (
        <Button type="button" variant="secondary" size="sm" className="shrink-0 self-end" onClick={() => onSave(req())} disabled={!host} loading={saving} title="Save site">
          <Save size={13} />Save
        </Button>
      )}
      <Button type="submit" variant="primary" size="sm" className="shrink-0 self-end" loading={connecting}>
...
function Field({ label, children, className = "" }: { label: string; children: React.ReactNode; className?: string }) {
  return (
    <label className={`flex flex-col gap-0.5 ${className}`}>
      <span className="whitespace-nowrap text-[10px] font-medium uppercase tracking-wide text-subtle">{label}</span>
      {children}
    </label>
  );
}
